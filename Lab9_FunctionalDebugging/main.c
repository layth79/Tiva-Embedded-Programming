...
unsigned long SW1;
unsigned long SW2;
unsigned long prevPF4;
unsigned long prevPF1;
unsigned long prevPF0;
unsigned long newPF4;
unsigned long newPF1;
unsigned long newPF0;
...
void Delay(void){unsigned long volatile time;
  time = 71250; // 0.1sec orignally, but now is every 0.05sec
  while(time){
   time--;
  }
}
...
int main(void){  unsigned long i,last,now;
  ...
  while(1){ 
		//PF4 is SW1, PF0 is SW2, PF1 is output
    SW1 = GPIO_PORTF_DATA_R & 0x10;
		SW2 = GPIO_PORTF_DATA_R & 0x01;
		prevPF0 = GPIO_PORTF_DATA_R & 0x01;
		prevPF1 = GPIO_PORTF_DATA_R & 0x02;
		prevPF4 = GPIO_PORTF_DATA_R & 0x10;
		if(!SW1 || !SW2 || (!SW1 && !SW2))
			GPIO_PORTF_DATA_R ^= 0x02;
		else
			GPIO_PORTF_DATA_R = 0x00; 
		Delay();
		newPF0 = GPIO_PORTF_DATA_R & 0x01;
		newPF1 = GPIO_PORTF_DATA_R & 0x02;
		newPF4 = GPIO_PORTF_DATA_R & 0x10;
		if(i<50){
			if((prevPF1 != newPF1) || (prevPF4 != newPF4) || (prevPF0 != newPF0)){ 
