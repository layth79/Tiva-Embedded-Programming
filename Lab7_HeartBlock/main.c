...
// Color    LED(s) PortF
// dark     ---    0
// red      R--    0x02, when vt is high
// blue     --B    0x04
// green    -G-    0x08, when ready is high
// yellow   RG-    0x0A
// sky blue -GB    0x0C
// white    RGB    0x0E
...
void Delay1ms(unsigned long msec){
	unsigned long i;
  while(msec > 0){
		i = (16000 * 25) / 30; //this is the adjusted version for when it runs on the real board
    //i = 16000;  // this number means 1 ms
    while(i > 0){
      i--;
    }
